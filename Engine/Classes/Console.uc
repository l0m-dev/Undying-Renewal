...
var localized string DoneSaveMessage;
									  ...
									  float tX, float tY, float tW, float tH, texture Tex ) 
{
	...
	C.SetPos( X, Y );
	C.DrawTileClipped( Tex, W, H, tX, tY, tW, tH);
}

function DrawTransitionScreen(Canvas C)
{
	...
	{
		if (Back[i] == None ) 
			Back[i]=texture(DynamicLoadObject(TransitionScreenName[i], class'texture'));
...
function CleanupTransitionScreen()
{
	local int i;
	...
	for ( i=0; i<6; i++ )
	{
		if ( Back[i] != None )
		{
			Viewport.Actor.UnloadTexture( Back[i] );
			Back[i] = None;
		}
	}
...
function DrawLevelAction( canvas C )
{
	...
	else if ( Viewport.Actor.Level.LevelAction == LEVACT_DoneSave )
		BigMessage = DoneSaveMessage;
	...
	if ( BigMessage != "" )
	{
		if(bUseTransitionScreen && (Viewport.Actor.Level.LevelAction == LEVACT_Loading))
		{
			C.SetPos( 0, 0 );
			DrawTransitionScreen( C );
		}
		else
		{
			if( Viewport.Actor.Level.LevelAction == LEVACT_Precaching )
			{
				C.SetPos( 0, 0 );
				C.DrawColor.R = 0;
				C.DrawColor.G = 0;
...
defaultproperties
{
     ...
     DoneSaveMessage="SAVE COMPLETE"
     ConnectingMessage="CONNECTING"
     PausedMessage="PAUSED"
     PrecachingMessage="LOADING"
