...
singular event BaseChange()
{
	...
	if ( (Base != None) && Base.IsA('Mover') )
	{
		...
		if ( Mover(Base).bDamageTriggered && !Mover(Base).bOpening
			&& (MoveTarget != None) )
		{
			HitActor = Trace(HitLocation, HitNormal, HitJoint, MoveTarget.Location, Location, true);
			if ( HitActor == Base )
			{
				Target = Base;
				bShootSpecial = true;
...
function PainTimer()
{
	...
	{
		if (bDrowning)
			{}//self.TakeDamage(4.0, None, Location, vect(0,0,0)); 
		else
		{
			bDrowning = true;
			GotoState('FindAir');
...
function bool NearWall(float walldist)
{
	...
	HitActor = Trace(HitLocation, HitNormal, HitJoint, ViewSpot + ViewDist, ViewSpot, false);
	if ( HitActor == None )
		return false;
...
function bool CheckFutureSight(float deltatime)
{
	...
	HitActor = Trace(HitLocation, HitNormal, HitJoint, FutureLoc, Location, false);
	if ( HitActor != None )
		return false;
...
function rotator AdjustToss(float projSpeed, vector projStart, int aimerror, bool leadTarget, bool warnTarget)
{
	...
	if ( leadTarget )
	{
		...
		HitActor = Trace(HitLocation, HitNormal, HitJoint, FireSpot, ProjStart, false);
		if (HitActor != None)
...
function rotator AdjustAim(float projSpeed, vector projStart, int aimerror, bool leadTarget, bool warnTarget)
{
	...
	{
		// Try to aim at feet
 		HitActor = Trace(HitLocation, HitNormal, HitJoint, FireSpot - vect(0,0,80), FireSpot, false);
		if ( HitActor != None )
		{
...
function TryToDuck(vector duckDir, bool bReversed)
{
	...
	HitActor = Trace(HitLocation, HitNormal, HitJoint, Location + 240 * duckDir, Location, false, true, extent);
	...
	HitActor = Trace(HitLocation, HitNormal, HitJoint, HitLocation - MaxStepHeight * vect(0,0,1), HitLocation, false, true, extent);
	if (HitActor == None)
		return;
	...
	if ( (Weapon != None) && Weapon.bSplashDamage
		&& (bFire != 0) && (Enemy != None) )
	{
		HitActor = Trace(HitLocation, HitNormal, HitJoint, Enemy.Location, HitLocation, false);
		if ( HitActor != None )
		{
			HitActor = Trace(HitLocation, HitNormal, HitJoint, Enemy.Location, Location, false);
			if ( HitActor == None )
			{
				bFire = 0;
			}
...
function bool TryToCrouch()
{
	...
	if ( (Dir2D Dot Vector(Rotation)) < 0.8 )
		return false;
	HitActor = Trace(HitLocation, HitNormal, HitJoint, StartSpot + 100 * ViewDir ,StartSpot, false);
	if ( HitActor == None )
		return false;
	bCrouching = true;
	return true;
...
function bool CanStakeOut()
{
	...
	HitActor = Trace(HitLocation, HitNormal, HitJoint, LastSeenPos, Location + EyeHeight * vect(0,0,1), false);
	if ( HitActor == None )
	{
		HitActor = Trace(HitLocation, HitNormal, HitJoint, LastSeenPos , Enemy.Location + Enemy.BaseEyeHeight * vect(0,0,1), false);
...
function bool CanFireAtEnemy()
{
	...
	if ( Weapon.bInstantHit )
		HitActor = Trace(HitLocation, HitNormal, HitJoint, Enemy.Location + Enemy.CollisionHeight * vect(0,0,0.7), projStart, true);
	else
		HitActor = Trace(HitLocation, HitNormal, HitJoint, 
...
state Roaming
{
	...
	function TakeDamage(Pawn instigatedBy, Vector hitlocation, 
							Vector momentum, DamageInfo damageType)
	{
		Global.TakeDamage(instigatedBy, hitlocation, momentum, damageType);
		if ( health <= 0 )
			return;
	...
	{
		/*
		if ( (AmbushSpot == None) && (AeonsAmbushPoint(MoveTarget) != None) )
			AmbushSpot = AeonsAmbushPoint(MoveTarget);
...
LongCamp:
	...
	Acceleration = vect(0,0,0);
	TweenToWaiting(0.15);
	//if ( OrderObject.IsA('AeonsAmbushPoint') )
	//	TurnTo(Location + (AeonsAmbushPoint(OrderObject)).lookdir);
	Sleep(CampTime);
	Goto('Begin');
...
state Wandering
{
	...
	{	
		...
		HitActor = Trace(HitLocation, HitNormal, HitJoint, Location + pick + 1.5 * CollisionRadius * dir , Location, false);
		if (HitActor != None)
		{
			pick = HitLocation + (HitNormal - dir) * 2 * CollisionRadius;
			HitActor = Trace(HitLocation, HitNormal, HitJoint, pick , Location, false);
			if (HitActor != None)
				return false;
		}
		else
...
ignores SeePlayer, HearNoise;
	...
	*/
	...
	{ 
		...
		Extent.Y = CollisionRadius;
		Extent.Z = CollisionHeight;
		HitActor = Trace(HitLocation, HitNormal, HitJoint, Location + 100 * sideDir, Location, false, true, extent);
		if (HitActor != None)
		{
			sideDir *= -1;
			HitActor = Trace(HitLocation, HitNormal, HitJoint, Location + 100 * sideDir, Location, false, true, extent);
		...
		if ( Physics == PHYS_Walking )
		{
			HitActor = Trace(HitLocation, HitNormal, HitJoint, Location + 100 * sideDir - MaxStepHeight * vect(0,0,1), Location + 100 * sideDir, false, true, extent);
			if ( HitActor == None )
				return false;
		}
...
ignores SeePlayer, HearNoise;
	...
	function bool ValidRecovery()
	{
		...
		HitActor = Trace(HitLocation, HitNormal, HitJoint, Enemy.Location, LastSeeingPos, false);
		return (HitActor == None);
	}
	...
	function bool TryToward(inventory Inv, float Weight)
	{
		...
		HitActor = Trace(HitLocation, HitNormal, HitJoint, minDest, Location, false, true, collSpec);
		if (HitActor == None)
		{
			...
			if ( !success )
			{
				...
				HitActor = Trace(HitLocation, HitNormal, HitJoint, minDest - (18 + MaxStepHeight) * vect(0,0,1), minDest, false, true, collSpec);
	...
	function PickRegDestination(bool bNoCharge)
	{
		...
		else 
		{
			...
			if ( enemyDist > minDist + 2 * CollisionRadius )
			{
				...
				HitActor = Trace(HitLocation, HitNormal, HitJoint, Destination + 2 * CollisionRadius * pickdir, Location, false);
...
ignores EnemyNotVisible; 
	...
	*/
	...
	function PickDestination()
	{
		...
		HitActor = Trace(HitLocation, HitNormal, HitJoint, LastSeenPos, ViewSpot, false);
		bCanSeeLastSeen = (HitActor == None);
		if ( bCanSeeLastSeen )
		{
			HitActor = Trace(HitLocation, HitNormal, HitJoint, LastSeenPos, Enemy.Location, false);
		...
		if ( LastSeeingPos != vect(1000000,0,0) )
		{
			...
			HitActor = Trace(HitLocation, HitNormal, HitJoint, Enemy.Location, ViewSpot, false);
			if ( HitActor == None )
			{
		...
		nextSpot.Z = posZ;
		HitActor = Trace(HitLocation, HitNormal, HitJoint, nextSpot , ViewSpot, false);
		if ( HitActor == None )
	...
	function bool FindViewSpot()
	{
		...
		HitActor = Trace(HitLocation, HitNormal, HitJoint,Enemy.Location, Location + 2 * Y * CollisionRadius, false);
		if ( HitActor == None )
		{
...
ignores EnemyNotVisible; 
	...
	function BeginState()
	{
		...
		Acceleration = vect(0,0,0);
		bCanJump = false;
 		HitActor = Trace(HitLocation, HitNormal, HitJoint,LastSeenPos + vect(0,0,0.9) * Enemy.CollisionHeight, Location + vect(0,0,0.8) * EyeHeight, false);
		bClearShot = (HitActor == None);
		bReadyToAttack = true;
		SetAlertness(0.5);
	}
...
ignores SeePlayer, EnemyNotVisible, HearNoise, Died, Bump, Trigger, HitWall, HeadZoneChange, FootZoneChange, ZoneChange, Falling, WarnTarget, LongFall, SetFall, PainTimer;
	...
	{
		if ( !bHidden )
			Super.TakeDamage(instigatedBy, hitlocation, momentum, damageType);
	}
