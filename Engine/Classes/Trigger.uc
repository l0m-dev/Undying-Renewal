...
function CheckTouchListAccurate()
{
	local Actor A;
...
function Touch( actor Other )
{
	...
	if( IsRelevant( Other ) )
	{
		...
		if( bTriggerOnceOnly )
		{
			...
			SetCollision(False);
			bInitiallyActive = false;
			// log("Setting bInitiallyActive to FALSE ....", 'Misc');
			Destroy();
		} else if ( RepeatTriggerTime > 0 )
			SetTimer(RepeatTriggerTime + RandRange(0.0, RepeatRandomTime), false);
	}
}
...
function TakeDamage( Pawn instigatedBy, Vector hitlocation, Vector momentum, DamageInfo DInfo)
{
	...
	if ( bInitiallyActive && (TriggerType == TT_Shoot) && (DInfo.Damage >= DamageThreshold) && (instigatedBy != None) )
	{
		if ( ReTriggerDelay > 0 )
		{
			if ( Level.TimeSeconds - TriggerTime < ReTriggerDelay )
