...
state() DamageInstigator
{
	function Trigger( actor Other, pawn EventInstigator )
	{
		...
		if ( Other.AcceptDamage(DInfo) )
			Other.TakeDamage( EventInstigator, EventInstigator.Location, Vect(0,0,0), getDamageInfo() );
	}
}
...
state() KillInstigator
{
	function Trigger( actor Other, pawn EventInstigator )
	{
		...
		if( EventInstigator != None )
			EventInstigator.Died( None, DamageType, EventInstigator.Location, DInfo );
	}
}
...
state() PlaySoundEffect
{
	...
	{
		Global.Trigger( Self, EventInstigator );
		//PlaySound( Sound,,Float(SoundVolume)/128,,Float(SoundRadius*SoundRadius) );
		PlaySound( Sound,,SoundVolume,,Float(SoundRadius*SoundRadius),SoundPitch);
	}
}
