...
//=============================================================================
class Inventory extends Visible
	abstract
	native
	nativereplication;
...
var	  bool		  bHaveTokens;		  // true if item still has tokens (bullets, etc.)
var() name		ConditionalEvent;
var() bool		bActiveToggle;			// this item toggles between active and inactive
//-----------------------------------------------------------------------------
...
var   rotator	AutoAimDir;			// used by the player when using auto-aim
...
var(MuzzleFlash3rd) bool bMuzzleFlashParticles;
var(MuzzleFlash3rd) bool bToggleSteadyFlash;
var bool	bSteadyToggle;
var byte FlashCount, OldFlashCount;
var(MuzzleFlash) ERenderStyle MuzzleFlashStyle;
var(MuzzleFlash3rd) mesh MuzzleFlashMesh;
var(MuzzleFlash3rd) float MuzzleFlashScale;
var(MuzzleFlash3rd) texture MuzzleFlashTexture;
...
{
    if ( Inventory != None )
        return Inventory.RecommendAttSpell(rating, bUseAltMode);
    else
    {
        rating = -1;
        return None;
...
{
    if ( Inventory != None )
        return Inventory.RecommendDefSpell(rating, bUseAltMode);
    else
    {
        rating = -1;
        return None;
...
// Called to add saved inventory items onto the player
event LoadGameAdd(Pawn player)
{
	GiveTo(player);
}
...
function Inventory SelectNext()
{
	if ( bActivatable ) 
	{
		if (( M_Selected != "" ) && (PlayerPawn(Owner).bUpdateInventorySelect == True))
			Pawn(Owner).ClientMessage(ItemName$M_Selected);
		if ( self.IsA('Ammo') )
		{
			if ( Ammo(self).AmmoAmount > 0 )
				return self;
		} else {
			return self;
		}
...
auto state Pickup
{
	...
	function SendEvents( actor Other )
	{
		local Actor A;
			...
			foreach AllActors( class 'Actor', A, Event )
			{
				if ( A.IsA('Trigger') )
				{
					if ( Trigger(A).bPassThru )
						Trigger(A).PassThru(Other.Instigator);
				}
	...
	function bool ValidTouch( actor Other )
	{
		...
		retval = Other.bIsPawn && Pawn(Other).bIsPlayer && (Pawn(Other).Health > 0) && Level.Game.PickupQuery(Pawn(Other), self);
