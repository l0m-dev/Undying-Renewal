...
function InitShame()
{
	if ( IconsofShame == None ) 
	{
		Log("Initializing Icons of Shame...");
	
		IconsofShame = spawn(class'StatManager');
	}
	else
		Log("Icons of Shame already initialized");
}
*/
			...
			if ( P.bIsPlayer && (P != NewPlayer) )
			{
				...
				if (ppawn != None)
				{
					if ( P.bIsMultiSkinned )
						NewPlayer.ClientReplicateSkins(ppawn.MultiSkins[0], ppawn.MultiSkins[1], ppawn.MultiSkins[2], ppawn.MultiSkins[3]);
					else
