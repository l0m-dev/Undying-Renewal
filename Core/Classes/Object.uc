...
// Convert degrees to Rotator values.
const DEGREES = 182;
...
struct Lighting						// Lighting properties for a material.
{
	var() color	Constant;			// Constant (self-illumination) color.
	var() color	Diffuse;			// Omni-directional reflectance (modulates texture).
	var() color	SpecularShade;		// Directional reflectance, shading texture.
	var() color	SpecularHilite;		// Directional reflectance, highlight texture.
	var() byte	SpecularWidth;		// Sharpness: 0 = perfectly sharp, max = perfectly diffuse.
	var() int   TextureMask;		// Bitmask of textures this material applies to. -1 = all.
};
...
// Quat functions.
native(330) static final function quat ConvertRot ( rotator R );
native(331) static final function rotator ConvertQuat ( quat Q );
native(332) static final function quat RotateTo   ( vector From, vector To, optional vector Axis );
...
// Logging.
native(286) final static function LogStack( optional name Tag );
...
native(239) final static function EnableLog( name Filename );
...
// Timing
native(197) final function Clock( out float Timer );
native(198) final function UnClock( out float Timer );
native(199) final function float GetFrameTime();

// Platform
const PLATFORM_WinPC = 0;
const PLATFORM_PSX2  = 1;

native(205) final function int GetPlatform();
native(2007) final function EnableStates();
