...
var bool AnalogDown;	// is analog outside dead zone?
		...
		case IST_Press:
			switch(k)
			{
				...
				case EInputKey.IK_Joy16:
					...
					if ( bShellPauses )
					{
						Log("bShellPauses =  True");
						if ( Viewport.Actor.Level.Pauser == Viewport.Actor.PlayerReplicationInfo.PlayerName )
						{
							Viewport.Actor.Level.Pauser = "";
						} 
						else 
						{
							Viewport.Actor.Level.Pauser = Viewport.Actor.PlayerReplicationInfo.PlayerName;
						}
						return true;
					}
...
event Tick( float Delta )
{
	...
	// update our delay for holding ESC down
	if ( EscapeDelay > 0.0 )
		EscapeDelay -= Delta;
	...
	{		
		if ( GetPlatform() == PLATFORM_PSX2 )
		{
			...
			RootWindow = "BootShellPSX2.AeonsRootWindowPSX2";
			bQuickKeyEnable = False;
			ShowDesktop = True;
			bNoDrawWorld = True;
			LaunchUWindow();
...
function LaunchUWindow()
{
	...
	ViewPort.Actor.GUIEnter(ViewPort);
	...
	if ( GetPlatform() == PLATFORM_PSX2 )
	{
		// turn analog stick into a digital stick
		ViewPort.Actor.bDigitalStickPSX2 = True;
//		Log("bDigitalStick = "$ViewPort.Actor.bDigitalStickPSX2);
	}
...
function CloseUWindow()
{
	...
	if ( GetPlatform() == PLATFORM_PSX2 )
	{
		// turn analog stick functionality back on
		ViewPort.Actor.bDigitalStickPSX2 = False;
//		Log("bDigitalStick = "$ViewPort.Actor.bDigitalStickPSX2);
	}
